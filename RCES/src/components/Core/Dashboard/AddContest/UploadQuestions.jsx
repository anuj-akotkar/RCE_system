...
export default function UploadQuestions({ onNext }) {
  ...
  return (
    <div className="max-w-5xl mx-auto bg-white text-gray-800 rounded-xl shadow-lg p-8 mt-6">
      ...
      {/* Questions List */}
      <div>
        <h2 className="text-lg font-semibold mb-4">Questions Added</h2>
        {questions.length === 0 && (
          <div className="text-gray-500 mb-4">No questions added yet.</div>
        )}
        <ul className="space-y-3">
          {questions.map((q, idx) => (
            <li key={idx} className="flex flex-col md:flex-row items-start md:items-center justify-between bg-gray-50 px-4 py-3 rounded-lg shadow">
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-800 break-words">{idx + 1}. {q.title}</p>
                <p className="text-sm text-gray-600">Difficulty: {q.difficulty}</p>
              </div>
              <div className="mt-2 md:mt-0 space-x-2 flex-shrink-0">
                <button
                  onClick={() => handleEdit(idx)}
                  className="px-4 py-1.5 bg-yellow-100 text-yellow-700 rounded hover:bg-yellow-200 text-sm"
                >
                  Edit
                </button>
                <button
      ...
      <div className="flex justify-end mt-8">
        <button
          ...
          className={`px-8 py-3 rounded-lg font-bold text-lg ${isValidStep ? "bg-green-600 text-white hover:bg-green-700" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}
        >
          Next
        </button>
      </div>
