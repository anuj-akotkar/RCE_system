      ...
      {output.success ? (
        <div>
          ...
          {output.results && Array.isArray(output.results) && (
            <div className="mb-4">
              <h3 className="text-md font-medium mb-2 text-yellow-400">
                Sample Test Cases
              </h3>
              {output.results.map((res, i) => (
                <div key={i} className="mb-3 p-2 border border-gray-600 rounded">
                  ...
                  <div className="ml-4 text-sm">
                    <div>
                      <strong>Input:</strong>{" "}
                      <span className="text-blue-300">{res.input}</span>
                    </div>
                    <div>
                      <strong>Expected:</strong>{" "}
                      <span className="text-green-300">{res.expectedOutput}</span>
                    </div>
                    <div>
                      <strong>Got:</strong>{" "}
              ...
              <div className="p-3 border border-gray-600 rounded bg-gray-800">
                  ...
                  <span
                    className={`font-bold ${
                      output.submission.status === "Accepted"
                        ? "text-green-400"
                        : output.submission.status === "Wrong Answer"
                        ? "text-red-400"
                        : "text-yellow-400"
                    }`}
                  >
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2 text-white">
                      {formatMemory(output.submission.maxMemory)}
                    </span>
                  </div>
                  <div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="max-h-40 overflow-y-auto">
                      {output.submission.results.map((res, i) => (
                        <div
                          key={i}
                          className="mb-2 p-2 border border-gray-700 rounded text-xs"
                        >
                          ...
                          {!res.passed && (
                            <div className="mt-1 text-gray-300">
                              ...
                              <div>
                                Got:{" "}
                                <span className="text-red-300">{res.actualOutput}</span>
                              </div>
                              {res.status && res.status !== "Unknown" && (
                                <div>
                                  Status:{" "}
                                  <span className="text-yellow-300">{res.status}</span>
                                </div>
                              )}
                              {res.error && (
                                <div>
